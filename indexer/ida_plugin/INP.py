...
class _FuncExportJob(object):
    ...
    def __init__(self, export_dir, skip_existing=True, force_reexport=False, max_functions=0, timeout=0):
        ...
        self.max_functions = max_functions  # 0 = unlimited, >0 = limit decompiled function count
        self.timeout = timeout              # 0 = unlimited, >0 = overall timeout in seconds
    ...
    def run_blocking(self, show_dialog=False):
        ...
        try:
            ...
            # Apply max_functions limit: sort by size (smallest first) and take top N
            if self.max_functions > 0 and len(self.remaining_funcs) > self.max_functions:
                original_count = len(self.remaining_funcs)
                # Sort by function size (smallest first) for best coverage within limit
                sized_funcs = []
                for ea in self.remaining_funcs:
                    func = ida_funcs.get_func(ea)
                    size = (func.end_ea - func.start_ea) if func else 0
                    sized_funcs.append((ea, size))
                sized_funcs.sort(key=lambda x: x[1])
                self.remaining_funcs = [ea for ea, _ in sized_funcs[:self.max_functions]]
                logger.info("max_functions=%d: selected %d/%d functions (smallest first)",
                            self.max_functions, len(self.remaining_funcs), original_count)
            ...
            while self.idx < len(self.remaining_funcs):
                ...
                # Timeout check
                if self.timeout > 0 and (time.time() - self._job_start_time) > self.timeout:
                    logger.warning("Overall timeout (%ds) reached after %d/%d functions, stopping",
                                   self.timeout, self.idx, len(self.remaining_funcs))
                    break
...
def export_decompiled_functions_sync(export_dir, skip_existing=True, force_reexport=False, max_functions=0, timeout=0):
    """阻塞式导出函数，用于批处理模式。"""
    ensure_dir(os.path.join(export_dir, "decompile"))
    ensure_dir(os.path.join(export_dir, "disassembly"))
...
def do_export_sync(export_dir=None, skip_auto_analysis=False, worker_count=None, force_reexport=False, max_functions=0, timeout=0):
    """同步阻塞导出，适用于 `idat -A -S...` 批处理模式。

    Args:
        max_functions: Maximum number of functions to decompile (0=unlimited).
                       When set, smallest functions are prioritized for best coverage.
        timeout: Overall timeout in seconds for decompilation phase (0=unlimited).
                 When exceeded, already-exported functions are kept and the job finishes.
    """
    global WORKER_COUNT
    ...
    try:
        ...
        export_decompiled_functions_sync(export_dir, skip_existing=True, force_reexport=force_reexport, max_functions=max_functions, timeout=timeout)
